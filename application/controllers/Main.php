...
class Main extends CI_Controller
{
	...
	public function index()
	{
		...
		// query for chart
		$data['query'] = $this->db->query('SELECT count(tbt.id_menu) as total_laku, tbo.*, tbm.nama_menu, SUM(tbp.total_pembayaran) as harga_laku
		FROM tb_transaksi tbt, tb_outlet tbo, tb_menu tbm, tb_pembayaran tbp
		WHERE tbt.id_menu = tbm.id_menu
		AND tbt.id_outlet = tbo.id_outlet
		AND tbt.kode_invoice = tbp.kode_invoice 
		GROUP BY tbt.id_menu')->result_array();
		// data stok
		$data['stok_minim'] = $this->db->query('SELECT nama_menu, stok FROM tb_menu WHERE stok <= 3')->result_array();
